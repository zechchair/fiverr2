...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  if(searchParam === "undefined") return {redirect: {permanent: false, destination: "/admin/logs"}};
  ...
  const logs: Log[] = (
    ...
    const {user}: {user: User, payment: Payment, product: Product} = JSON.parse(log.defaultData);
    const firstname = user.firstname
      ?.toLowerCase()
      .includes(searchParam as string);
...
class Logs extends React.Component<props, state> {
  ...
  async search(e) {
    ...
    window.location.href = `/admin/logs/${search}`;
  }
